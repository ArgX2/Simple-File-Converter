...
완전한 보존이 어려운 항목은 프로그램에서 `XXX로 불완전 변환` 형식으로 표시합니다. 불완전 변환은 결과 품질을 보장하지 않으며, 일부 내용·서식·배치가 누락되거나 제대로 변환되지 않을 수 있습니다. 실제 변환 실패와 구분되는 품질 안내입니다.

### 변환 가능 여부

`O`는 일반 변환, `△`는 일부 내용·품질·구조가 달라질 수 있는 불완전 변환, `X`는 지원하지 않는 변환입니다.

| 입력 \ 출력 | 이미지 | 영상 | 음성 | PDF | PPTX | PPT |
| --- | --- | --- | --- | --- | --- | --- |
| PNG/JPG/GIF 등 이미지 | O/△ | △ | X | O | X | X |
| MP4/AVI 등 영상 | △ | △ | △ | X | X | X |
| MP3/WAV 등 음성 | X | X | △ | X | X | X |
| PDF | △ | X | X | X | △ | △ |
| PPT/PPTX | X | X | X | △ | X | X |

세부적으로 선택 가능한 출력 형식은 위의 변환 형식 표를 따르며, 파일 내용과 설치된 Office 프로그램에 따라 달라질 수 있습니다.
