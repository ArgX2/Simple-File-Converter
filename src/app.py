...
class DropZone(QLabel):
    files = Signal(list)
    def __init__(self):
        super().__init__(tr('여기에 파일 드래그 혹은 파일 선택'))
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.context_menu = QCheckBox(tr('우클릭 변환 메뉴 사용'))
        self.context_menu.setToolTip(tr('Windows 11: 더 많은 옵션 표시 → Simple File Converter\n등록 후 실행 파일을 옮기면 체크를 껐다 켜 주세요.'))
        self.context_menu.setChecked(is_enabled())
        self.context_menu.toggled.connect(self.toggle_context_menu)
        footer = QHBoxLayout()
        self.language_label = QLabel(tr('언어'))
        footer.addWidget(self.language_label)
        self.language_picker = QComboBox()
        self.language_picker.setAccessibleName(tr('언어'))
        self.language_picker.addItem(tr('자동 (시스템 언어)'), 'auto')
        for code, name in LANGUAGES.items():
            self.language_picker.addItem(name, code)
        self.language_picker.setCurrentIndex(max(0, self.language_picker.findData(selection())))
    ...
    def change_language(self):
        previous = selection()
        try:
            set_language(self.language_picker.currentData(), persist=True)
        except OSError as exc:
            set_language(previous)
            self.language_picker.blockSignals(True)
            self.language_picker.setCurrentIndex(self.language_picker.findData(previous))
            self.language_picker.blockSignals(False)
            self.notify(tr('언어 설정을 저장하지 못했습니다'), str(exc), 'error')
            return
        # Expired notification text need not survive a language switch.
        for toast in list(self.toasts.items): self.toasts.remove(toast)
        retranslate(self)
        self.language_picker.setItemText(0, tr('자동 (시스템 언어)'))
        self.language_picker.setAccessibleName(tr('언어'))
        ...
        if self.entries and all(entry['state'] != '대기' for entry in self.entries):
            ...
            self.status.setText(tr('{v0}개 파일', v0=len(self.entries)) if self.entries else tr('파일 대기'))
        if self.context_menu.isChecked():
            self.toggle_context_menu(True)
    def toggle_context_menu(self, checked):
        ...
        try:
            ...
            self.notify(tr('우클릭 메뉴를 변경하지 못했습니다'), tr('상세 내용을 확인한 뒤 다시 시도해 주세요.'), 'error', str(exc))
            return
        if checked:
            self.notify(tr('우클릭 변환 메뉴를 켰습니다'), tr('파일을 우클릭하고 변환 형식을 선택해 주세요.'))
        else:
            self.notify(tr('우클릭 변환 메뉴를 껐습니다'), tr('필요할 때 다시 켤 수 있습니다.'))
    ...
    def reset_drag(self):
        ...
        self.drop.setText(tr('여기에 파일 드래그 혹은 파일 선택'))
    ...
    def eventFilter(self, obj, event):
        ...
        if kind in (QEvent.DragEnter, QEvent.DragMove):
            ...
            self.drop.setText(tr('변환 중 · 추가 불가') if blocked else tr('{v0}개 파일 감지 · 놓아서 추가', v0=sum((u.isLocalFile() for u in urls))))
        ...
        if kind == QEvent.Drop:
            ...
            self.reset_drag()
            if self.busy(): self.notify(tr('변환 중입니다'), tr('완료 후 파일을 추가해 주세요.'))
    ...
    def add_files(self, paths):
        ...
        if slides:
            ...
            if name and self.presentation_consent:
                ...
                answer = QMessageBox.question(self, tr('PPT 변환 확인'), tr('{v0}가 설치되어 있습니다. 파일을 PDF 변환 목록에 추가할까요?\n\nPDF를 PPT로 바꾸면 이미지 슬라이드로 저장되어 텍스트를 개별 편집할 수 없습니다.\n예를 누르면 목록을 비울 때까지 다시 묻지 않습니다.', v0=name), QMessageBox.Yes | QMessageBox.No)
                allowed = answer == QMessageBox.Yes
                if allowed: self.presentation_consent = True
            else:
                QMessageBox.warning(self, tr('변환 프로그램이 필요합니다'), tr('PowerPoint 또는 LibreOffice를 설치한 뒤 다시 시도해 주세요.'))
                allowed = False
        ...
        for value in paths:
            ...
            if str(path).lower() in known:
                ...
                continue
            if not path.is_file():
                errors.append(tr('{v0}: 파일을 선택해 주세요.', v0=path.name))
                continue
            try:
                meta = inspect(path)
            except Exception as e:
        ...
        self.status.setText(tr('{v0}개 파일', v0=len(self.entries)))
        self.start.setEnabled(bool(self.entries))
        if not self.entries: self.presentation_consent = False
        if added: self.notify(tr('파일을 추가했습니다'), tr('{v0}개 파일의 변환 형식을 선택해 주세요.', v0=added))
        if duplicates: self.notify(tr('이미 추가된 파일입니다'), tr('중복된 {v0}개 파일은 건너뛰었습니다.', v0=duplicates))
        if errors: self.notify(tr('일부 파일을 추가하지 못했습니다'), tr('{v0}개 파일의 원인을 상세 보기에서 확인해 주세요.', v0=len(errors)), 'error', '\n'.join(errors))
    def remove_selected(self):
        ...
        self.status.setText(tr('{v0}개 파일', v0=len(self.entries)))
        if not self.entries: self.presentation_consent = False
        if count: self.notify(tr('목록에서 제거했습니다'), tr('{v0}개 항목을 제거했습니다. 원본 파일은 그대로입니다.', v0=count))
    def clear_files(self):
        self.entries.clear()
        self.presentation_consent = False
        self.table.setRowCount(0)
        self.start.setEnabled(False)
        self.status.setText(tr('파일 대기'))
        self.progress.setValue(0)
        self.notify(tr('목록을 비웠습니다'), tr('새 파일을 추가해 주세요.'))
    def choose_folder(self):
        value = QFileDialog.getExistingDirectory(self, tr('저장 폴더 선택'), self.folder.text())
        if value:
            self.folder.setText(value)
            self.notify(tr('저장 위치를 변경했습니다'), tr('선택한 폴더에 결과를 저장합니다.'))
    def open_folder(self):
        ...
        else: self.notify(tr('저장 폴더가 아직 없습니다'), tr('변환을 시작하면 자동으로 생성됩니다.'))
    def set_busy(self, busy):
        for widget in [self.pick, self.remove, self.clear, self.drop, self.same_folder, self.start, self.language_picker]: widget.setEnabled(not busy)
    ...
    def begin(self):
        ...
        if not value and not self.same_folder.isChecked():
            self.notify(tr('저장 위치를 선택해 주세요'), tr('변환 결과를 저장할 폴더가 필요합니다.'), 'error')
            return
        ...
        self.status.setText(tr('변환 중 · 0/{v0}개 처리', v0=len(jobs)))
        self.worker = Worker(jobs, folder)
        self.worker.status.connect(self.update_status)
        self.worker.result.connect(self.receive)
        self.worker.finished.connect(self.finished)
        self.worker.start()
        self.notify(tr('변환을 시작했습니다'), tr('{v0}개 파일을 처리하고 있습니다.', v0=len(jobs)))
    def update_status(self, row, text):
        self.table.item(row, 3).setText(tr(text))
    ...
    def receive(self, row, state, result):
        ...
        self.done_count += 1
        self.status.setText(tr('변환 중 · {v0}/{v1}개 처리', v0=self.done_count, v1=len(self.entries)))
    ...
    def office_login(self):
        if QMessageBox.question(self, tr('Office 로그인 확인'), tr('Office 연결에 문제가 있습니다. 로그인 확인을 위해 브라우저를 열까요?\n\nWindows 세션 오류라면 Office 앱을 다시 실행해야 할 수 있습니다.'), QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
            if QDesktopServices.openUrl(QUrl('https://www.office.com/login')):
                self.notify(tr('로그인 페이지를 열었습니다'), tr('로그인을 마친 뒤 변환을 다시 시도해 주세요.'))
            else:
                self.notify(tr('브라우저를 열지 못했습니다'), tr('브라우저에서 office.com/login에 접속해 주세요.'), 'error')
    def finished(self):
        self.set_busy(False)
        ...
        self.status.setText(' · '.join(tr('{v0} {v1}개', v0=tr(state), v1=count) for state, count in counts.items()))
        if counts['실패']:
            self.notify(tr('일부 파일을 변환하지 못했습니다'), self.status.text() + tr('\n실패 항목을 두 번 클릭해 원인을 확인해 주세요.'), 'error')
        elif counts['취소됨']:
            self.notify(tr('변환을 취소했습니다'), self.status.text())
        else:
            self.notify(tr('변환을 완료했습니다'), tr('{v0}개 파일을 저장했습니다. 저장 폴더에서 확인해 주세요.', v0=counts['완료']), 'success')
    def stop(self):
        if self.busy():
            self.worker.cancel.set()
            self.cancel.setEnabled(False)
            self.status.setText(tr('취소 중…'))
            self.notify(tr('취소하고 있습니다'), tr('작업을 정리하고 있습니다. 잠시만 기다려 주세요.'))
    def show_result(self, row, col):
        entry = self.entries[row]
        if entry['state'] == '완료': QDesktopServices.openUrl(QUrl.fromLocalFile(entry['result']))
        elif entry['state'] == '실패': QMessageBox.warning(self, tr('변환 오류 안내'), entry['result'])
    def closeEvent(self, event):
        if self.busy():
            self.stop()
            self.status.setText(tr('취소 중 · 완료 후 종료'))
            event.ignore()
        else:
...
STYLE = '''
QWidget { background: #f5f7fb; color: #20283b; font-family: "Segoe UI", "Malgun Gothic"; font-size: 13px; }
