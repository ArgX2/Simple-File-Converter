...
class QuickConvert(QWidget):
    def __init__(self, path, fmt, worker_type):
        ...
        self.title = QLabel('파일을 변환하고 있습니다')
        ...
        self.details = QPushButton('상세 보기'); self.details.hide()
        self.details.clicked.connect(self.show_details); row.addWidget(self.details)
        self.cancel = QPushButton('취소'); self.cancel.clicked.connect(self.stop); row.addWidget(self.cancel)
